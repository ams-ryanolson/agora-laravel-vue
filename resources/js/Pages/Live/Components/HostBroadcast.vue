...
import {CogIcon, ChatBubbleLeftRightIcon, EllipsisVerticalIcon} from "@heroicons/vue/24/outline";
...
const join = async () => {
    console.log('joining')
    console.log(appId.value, channel.value, token.value, page.props.auth.user.id);
    await client.join(appId.value, channel.value, token.value, page.props.auth.user.id)
}
...
<template>
    <ChooseDevices v-if="openChooseDevices"
    ...
    <div
        ...
        <div class="w-full h-full rounded-lg relative" id="local-player">
            <div
                ...
                <div v-for="message in reverseMessages" :key="message.id" class="flex flex-row gap-1 w-full justify-between items-center">
                    <div class="flex flex-col gap-1">
                        <div class="text-xs text-gray-300">{{ message.username }}</div>
                        <div class="text-sm font-light text-gray-100">{{ message.message }}</div>
                    </div>
                    <Menu as="div" class="relative">
                        <MenuButton class="flex justify-center items-center">
                            <span class="sr-only">Open message menu</span>
                            <EllipsisVerticalIcon class="w-5 h-5 text-gray-300 cursor-pointer"/>
                        </MenuButton>
                        <transition enter-active-class="transition ease-out duration-100"
                            ...
                            <MenuItems class="z-50 absolute border border-gray-700 text-sm right-0 mt-2 w-40 origin-top-right bg-gray-900 divide-y divide-gray-600 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none overflow-auto">
                                <div class="px-1 py-2 ">
                                    <MenuItem as="div" v-for="item in messageNavigation" :key="item.id" v-slot="{ active }" class="px-2 py-1">
                                        {{ item.name }}
                                    </MenuItem>
                                </div>
                            </MenuItems>
        ...
        <div class="absolute top-0 left-0 w-full xs:h-12 md:h-16 bg-gray-900/50"></div>
        <a href="/live" class="absolute xs:top-3 md:top-4 left-4 font-logo flex flex-row xs:text-sm md:text-lg">
            <div class="text-white">REALKINK.</div>
            <div class="text-sky-500">MEN</div>
        </a>
        <div class="absolute xs:top-2 md:top-4 right-4">
                    <span v-if="isBroadcasting" class="rounded-2xl px-6 py-1.5 bg-red-600 font-medium text-white xs:text-sm md:text-lg"
                          :class="{'animate-pulse': showPulse}">LIVE</span>
        </div>
        <div
            class="w-full h-12 flex flex-row justify-between items-center absolute bottom-0 rounded-b-lg z-40"
            :class="{ 'bg-gray-900/80': mobileChatOverlay, 'bg-gray-900/50': !mobileChatOverlay }">
            <div class="flex flex-row gap-6 px-4 items-center">
                <div>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512"
                         class="hover:fill-green-700 h-5"
                         :class="{'fill-gray-600': !isBroadcasting, 'fill-green-600': isBroadcasting}"
                         v-if="!isVideoMuted"
                         @click="muteVideo">
                        <path
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512"
                         class="fill-red-600 h-5" v-else @click="unmuteVideo">
                        <path
                ...
                <div>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512"
                         class="fill-red-600 h-5" v-if="isAudioMuted" @click="unmuteAudio">
                        <path
                ...
                <CogIcon @click="openChooseDevices = true" class="w-6 h-6 text-gray-300 cursor-pointer"/>
                <ChatBubbleLeftRightIcon @click="mobileChatOverlay = !mobileChatOverlay"
                                         :class="mobileChatOverlay ? 'text-green-500' : 'text-red-500'"
                                         class="w-6 h-60 cursor-pointer xs:visible md:hidden"/>

            </div>
            <div class="flex flex-row gap-6 px-2 items-center">

                <button class="text-sm font-medium text-gray-100 bg-sky-500 hover:bg-sky-600 px-5 py-1 rounded-lg"
                        v-if="!isBroadcasting" @click="goLive">
                    Go Live
                </button>
                <button class="text-sm font-medium text-gray-100 bg-red-500 hover:bg-red-600 px-5 py-1 rounded-lg"
                        v-else @click="endLive">
                    End Live
                </button>
            </div>
        </div>
