<script setup>
import {ref, defineProps, defineEmits, watchEffect, reactive, watch} from 'vue';
import {
    Dialog,
    DialogPanel,
    DialogTitle,
...
const selectedAudioDevice = ref(props.audioDevices.length > 0 ? props.audioDevices[0].deviceId : null);
const selectedVideoDevice = ref(props.videoDevices.length > 0 ? props.videoDevices[0].deviceId : null);
                        ...
                        <DialogPanel
                            ...
                            <div class="flex flex-col">
                                <div class="text-center flex justify-between items-center">
                                    ...
                                    <button @click="closeModal" class="text-gray-300 hover:text-gray-100">
                            ...
                            <div class="flex flex-col gap-4 pt-4">
                                <div>
                                    <label for="audioDevices" class="block font-medium leading-6 text-gray-100">Audio Device</label>
                                    <select id="audioDevices"
                                        ...
                                        <option v-for="device in props.audioDevices" :key="device.deviceId"
                                                :value="device.deviceId">{{ device.label }}</option>
                                    </select>
                                </div>
                                <div>
                                    <label for="videoDevices" class="block font-medium leading-6 text-gray-100">Video Device</label>
                                    <select id="videoDevices"
                                            name="videoDevices"
                                            v-model="selectedVideoDevice"
                                            class="bg-gray-800 mt-2 block w-full rounded-md border-0 py-1.5 pl-3 pr-10 text-gray-100 ring-1 ring-inset ring-gray-700 focus:ring-2 focus:ring-sky-600 sm:text-sm sm:leading-6">                                        <option v-for="device in props.videoDevices" :key="device.deviceId"
                                                :value="device.deviceId">{{ device.label }}</option>
                                    </select>
                                </div>
